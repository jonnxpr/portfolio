...
- Recent workflow runs: `gh run list -R <owner/repo> --limit 30 --json status,conclusion,name,headBranch,createdAt,updatedAt,databaseId,displayTitle,url`
- Branch workflow status: `gh run list -R <owner/repo> --branch <branch> --limit 10 --json status,conclusion,name,updatedAt,databaseId,url`
- Failed workflow runs: `gh run list -R <owner/repo> --status failure --limit 30 --json status,conclusion,name,headBranch,updatedAt,databaseId,url`
...
- Never print tokens from `gh auth status`.
